"""Render with Pannini projection (canonical d=1) — fisheye/wide-angle variant.

Identical to render_pannini.py but camera filtering defaults to all cameras
(name_prefix='') and adds --name_contains for substring filtering.
...
def main():
    ...
    ap.add_argument('--name_prefix', default='')
    ap.add_argument('--name_contains', default='', help='additionally filter img_name by substring')
    ...
    keep = [c for c in cams_all
            if (not args.name_prefix or c['img_name'].startswith(args.name_prefix))
            and (not args.name_contains or args.name_contains in c['img_name'])]
